...
discrete promptingdml 경우 prompt token $\left\{ "It", "is", "[MASK]" \right \}\subset \mathcal{V}$이 movie review classify에 일반적으로 사용될 수 있다. 예를 들어 “Amazing movie!”라는 input text $\mathbf{x}$가 있을 때 input embedding은 $\text{\[} \mathbf{e}(\mathbf{x}),\mathbf{e}(\mathbf{"It"}),\mathbf{e}(\mathbf{"is"}), \mathbf{e}(\mathbf{"[MASK]"})\text{\]}$로 표현된다.
