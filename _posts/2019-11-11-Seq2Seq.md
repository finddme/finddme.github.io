...
$${ \theta }^{ * }\approx argmax P(Y\|X ;\theta) where X={{ x }_{ 1 }, { x }_{ 2 }, { x }_{ 3 } \dots { x }_{ n }}, Y={{ y }_{ 1 }, { y }_{ 2 }, { y }_{ 3 } \dots { y }_{ m }}$$
...
$${ h }_{ t }^{ src }= { RNN }_{ enc }({ emb }_{ src }({ x }_{ t }),{ h }_{ t-1 }^{ src })$$

$${ H }^{ src }=[{ h }_{ 1 }^{ src }; { h }_{ 2 }^{ src }; \dots ; { h }_{ n }^{ src }]$$
