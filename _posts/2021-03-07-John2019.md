...
코퍼스 내 문장 ${s}_{\ell}$과 
$\frac{1}{\left| s_{\ell} \right|}^2$ index$\ell$은 각 문장의 단어 쌍 수를 normalize한다. 중요한 것은 tree distance와 squared distance ${\lVert {h_i}-{h_j} \rVert}_
{B}^{2}$ 의 차이를 최소화해야 한다는 것이다. (실제 vector 거리 ${\lVert h_i - h_j \rVert}_
{B}$가 항상 실제 parse tree distance에서 벗어나지만 encoding된 tree정보는 일치하며, squared distance로 최적화하는 것이 가장 효과적이기 때문이다.)
