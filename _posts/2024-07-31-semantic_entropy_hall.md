...
Sampling Parameter로는 대표적으로 Top-K, Top-P, Temperature가 있는데 이들은 LLM의 출력을 제어하는 parameter이다. 해당 parameter를 통해 모델 출력의 일관성<->다양상 정도를 조절할 수 있다.
...
- Temperature scaling 작동 과정:
  1. model은 model이 사용하는 tokenizer에 속한 vocab에 있는 token을 현재 time step에 대한 next token으로 예측한다. 이때 각 token들에게는 정규화 되지 않은 log probability score가 부여된다.
