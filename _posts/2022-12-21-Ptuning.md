...
LM의 vocab을 $\mathcal{V}$, template을 구성하는 prompt의 $\text{i}$번째 token을 $\left[\text{P}_{i}\right]$라 하고 주어진 template이 위와 같을 때 discrete prompt search 방식의 경우 $\text{[}\text{P}_{i}\text{]}\in\mathcal{V}$이기 때문에(prompt를 구성하는 token이 LM vocab 안에 있기 때문에) discrete prompt template은 아래와 같이 표현될 수 있다.
