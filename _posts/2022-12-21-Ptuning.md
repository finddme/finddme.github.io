...
P-tuning은 선행 연구들의 문제를 해결하기 위해 위 그림과 같이 bi-LSTM으로 구성된 prompt encoder가 LM loss를 받아 학습하여 연속적인 prompt를 생성한다.  prompt encoder 학습 시에는 PLM의 전체 weight에 대해 parameter update(fine-tuning)를 하는 것이 아니라 continuous prompt embedding만 tuning한다.
