...
3. \<Client Agent> Agent Card를 기반으로 사용자 요청에 어떤 agent가 적합한지 점수를 매겨 호출할 agent를 선정함
  ...
  ```
5. \<Client Agent> Task 작성해서 remote agent에 전송
6. \<Server/Remote Agent> 요청을 받아 작업 수행 시작
7. \<All Agents>Task의 life cycle동안 agent들은 sse 혹은 websocket을 사용하여 Status, Artifact 등을 교환 (이 과정에서 실시간 피드백과 사용자 알림이 가능하다.)
   ```
   ...
   ```
8. \<Client Agent>가 상태가 completed이 되고 final:true 가 찍힌 이벤트를 받으면 모든 Artifact를 합쳐 사용자에게 반환하거나 다음 Agent에 전달
9. Agent Card와 Task 메타데이터를 캐시해 다음 호출 속도를 높임
