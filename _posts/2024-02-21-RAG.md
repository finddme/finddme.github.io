...
1) query reformulation: 사용자의 의도와 더 가깝게 질문을 재작성하는 방법
2) query expansion: 동의어나 관련 표현들을 통해 질문을 확장함으로써 관련있는 chunk 검색을 돕는 방법
3) query normalization: 철자 수정, 용어 차이 완화, 특수문자 및 공백 제거 불용어 제거 등 query 정규
...
**4. 쿼리 변환**  
쿼리 변환은 LLM을 추론 엔진으로 사용해 사용자 입력을 수정하여 검색 품질을 개선하는 일련의 기술입니다. 이를 수행하는 여러 가지 방법이 있습니다.  
쿼리가 복잡한 경우, LLM이 이를 여러 하위 쿼리로 분해할 수 있습니다. 예를 들어, 다음과 같은 질문을 한다면:  
- "Github에서 Langchain과 LlamaIndex 중 어떤 프레임워크가 별이 더 많은가?"  
우리의 데이터에서 직접적인 비교를 찾기 어려울 것이기 때문에, 이 질문을 더 간단하고 구체적인 정보 검색을 전제로 두 개의 하위 쿼리로 분해하는 것이 합리적입니다:  
- "Langchain은 Github에서 몇 개의 별을 가지고 있습니까?"  
- "LlamaIndex는 Github에서 몇 개의 별을 가지고 있습니까?"  

이 두 쿼리는 병렬로 실행된 후, 검색된 컨텍스트가 LLM을 위한 단일 프롬프트로 결합되어 초기 질문에 대한 최종 답변을 종합합니다.  
두 라이브러리 모두 이 기능을 구현하고 있습니다. Langchain에서는 Multi Query Retriever로, LlamaIndex에서는 Sub Question Query Engine으로 구현되어 있습니다.
