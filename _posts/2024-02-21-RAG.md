...
Advanced RAG은 Naive RAG의 문제점을 완화하기 위해 Retrieval 과정 혹은 Retrieval 앞뒤에 검색 정확도를 높이기 위한 과정이 추가된 RAG이다. 아래 그림에서 파란색 부분이 이에 해당한다.
...
검색 데이터(문서)가 많은 경우, 검색 효율을 높이기 위해 아래 그림과 같이 문서 요약 index와 문서 chunk index 단계를 만들어 우선 요약을 통해 관련 문서를 filtering한 후 해당하는 문서 chunk 중 질문과 의미 유사도가 높은 chunk들을 뽑는 방법이다.
