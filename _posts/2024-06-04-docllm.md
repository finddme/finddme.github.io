...
DocLLM은 unlabeled document에 대해 self-supervised 방식으로 사전학습 한다. 학습 과제는 text infilling objective로, 이를 수행하기 위해 사전 학습이 진행하는 동안 OCR 정보로부터 식별된 text block음 무작위로 masking되며 shuffle된다. block infilling은 시작과 끝을 나타내는 special token과 함께 autoregressively하게 수행된다. 이와 같은 방법은 사전학습 단계에서만 사용되고, fine-tuning 단계에서는 사용되지 않는다. 
